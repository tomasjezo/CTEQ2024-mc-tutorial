...
### Example 4: Fancier version of Example 2
The program from Example 2 (or 3) can be further improved:
...
### Example 5: Analysing $l^+l^-$ event sample with `Rivet`
...
![Alt text](pics/rivetScreenshot.png)
...
### Example 6: Using FIFO pip in Examples 2 and 5
...
### Example 7: Generating the $pp \to l^+ l^-$ process with `MG5_aMC@NLO`
...
![Alt text](pics/mg5Screenshot1.png)

---

### Example 8: Generating the $pp \to l^+ l^-$ event sample with `MG5_aMC@NLO`
...
![Alt text](pics/mg5Screenshot2.png)
...
### Example 9: Showering `MG5_aMC@NLO` events with `Pythia8`
...
### Example 10: Comparing `Pythia8` and `MG5_aMC@NLO+Pythia8` predictions
...
We note that the the `MG5_aMC@NLO` prediction is represented by a multitutde of lines. This is because it include scale and PDF variations in the `LHE` events -- each event contains one weight for each factorization and renormalization scale, and one weight for each PDF error member. Without further instructions on how to plot the weights, `Rivet3` simply renders all the lines. 
...
And like this, we can add a scale variation band constructed as an envelope of weights corresponding to a 7-pt scale variation for the default scale choice functional form: 
```bash
# This is bash
